...
then
    ...
    echo "File found but wrong operating system selected"
fi
if [ -f /etc/system-release ]
then
    echo "file exist"
    ret=$(egrep -i 'Amazon Linux release 2022' /etc/os-release)
    if [ $? -ne 0 ]
    then
        sudo yum update -y
...
then
    echo "file exist"
    ret=$(egrep -i 'Red Hat Enterprise Linux release' /etc/os-release)
    if [ $? -ne 0 ]
    then
        sudo yum update -y
       ...
       #This command will install tree
        ...
        echo "Red Hat Enterprise release 2 not found"
    fi
else
    echo "File found but wrong operating system selected"
fi